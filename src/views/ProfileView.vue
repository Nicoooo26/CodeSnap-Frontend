...
const emitControl = (mssg: string): void => {
  closeModal()
  if (mssg === 'ok') {
    loading.value = true
    getDataUser()
      ...
      <div class="lg:w-8/12 lg:mx-auto">
        <header class="flex flex-wrap items-center p-4 md:py-2">
          <div class="md:w-3/12 md:ml-16">
            <img
              class="w-20 h-20 md:w-40 md:h-40 object-cover border dark:bg-stone-100 border-stone-300 dark:border-stone-600 rounded-full p-1"
              :src="data.profilePicture" alt="profile" />
          </div>
          <div class="w-8/12 md:w-7/12 ml-4">
            <div class="md:flex md:flex-wrap md:items-center mb-4">
              <h2 class="text-3xl inline-block font-light md:mr-2 mb-2 sm:mb-0 dark:text-stone-50">{{ data.username }}
              </h2>
              <span
                class="inline-block fas fa-certificate fa-lg text-stone-500 relative mr-6 text-xl transform -translate-y-2"
                aria-hidden="true">
                <i class="fas fa-check text-stone-50 text-xs absolute inset-x-0 ml-1 mt-px"></i>
              </span>
              <Button v-if="isCurrentUser" label="Editar perfil"
                class="bg-stone-500 dark:bg-stone-700 px-2 py-1 text-stone-50 font-semibold text-sm rounded text-center sm:inline-block block"
                @click="openModal" />
            </div>
        ...
        <div class="px-px md:px-3">
          <ul
            class="flex md:hidden justify-around space-x-8 border-t border-stone-300 dark:border-stone-600 text-center p-2 text-stone-600 dark:text-stone-300 leading-snug text-sm">
            <li>
          ...
          <TabViewComponent @cerrar="getDataUser" v-bind="userId && { userId }" :numScripts="data.numCodes"
            :numPhotos="data.numPhotos" :numForums="data.numForums" />
        </div>
      </div>
    </main>
