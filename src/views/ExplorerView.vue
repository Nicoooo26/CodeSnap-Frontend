  ...
  <div class="flex flex-col items-center justify-start mt-8 bg-stone-200 text-stone-700 p-4">
    <div class="w-full max-w-xl mb-8 text-center">
      <input v-model="searchQuery" type="text" placeholder="Buscar usuario"
        class="w-3/4 p-3 border border-stone-500 rounded-l-lg focus:outline-none focus:ring-2 focus:ring-stone-800 text-lg" />
      <button @click="searchUsers"
        class="w-1/4 px-4 py-3 bg-stone-800 dark:bg-stone-400 dark:hover:bg-stone-600 text-white rounded-r-lg hover:bg-stone-700 focus:outline-none focus:bg-stone-700 transition duration-300 ease-in-out text-lg">Buscar</button>
    </div>
    <div v-if="!hasSearched" class="mt-6 w-full max-w-xl text-center text-stone-500">Introduzca el username que desea
      buscar</div>
    <div v-else class="mt-6 w-full max-w-xl text-center">
      ...
      <ul v-else class="space-y-4 max-h-96 overflow-y-auto">
        <div v-for="user in searchResults" :key="user.username" @click="goToProfile(user.id)">
          <li
            class="flex items-center p-4 bg-white rounded-lg shadow cursor-pointer hover:bg-stone-400 transition duration-200 border border-stone-600">
